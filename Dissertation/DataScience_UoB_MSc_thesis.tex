...
[To Do: add either architecture table (comparing the two or something that explains them visually]
To assess the quality of each denoiser, the noise level of each denoised signal was measured using MAD \ref{MAD noise estimate}. However, a reduction in noise level does not by itself indicate a better signal for spike sorting; focusing only on noise removal ignores whether the spike morphology is preserved \cite{baldazzi_systematic_2020}. However, \cite{baldazzi_systematic_2020} used RMSE to quantify morphology preservation against a clean reference. Here, the recording is real and noisy ground truth, so peak-to-peak was measured following\cite{adam_comparative_2025}. For each of the target spikes, the peak-to-peak amplitude was computed from the aligned waveforms. Amplitude retention was then expressed as the mean peak-to-peak amplitude of each denoised signal relative to the raw signal. 

\section{Classification (SVM)}
Two support vector machine (SVM) classifiers with radial basis function (RBF) kernels were applied to trough-aligned waveforms, following the raw waveform approach of \cite{troglio_hybrid_2026}. Both classifiers were evaluated using 5-fold cross-validation following \cite{troglio_hybrid_2026} with a fixed random seed. The candidates from the latency constraint survivors were split into 5 folds, and in each round, the model was trained on 4 folds and used to score the held-out fold. This validation ensures that no candidate is scored by a model trained on it to avoid information leakage. The out-of-fold scores were then pooled across all folds, and the metrics were computed once on the full pooled set to preserve the true target / non-target ratio. 
...
For metric evaluation, six metrics were employed, and each serves an evaluation purpose.
\subsection{Confusion matrix}
A confusion matrix is an evaluation table for classification models, which counts true positives (TP), false positives (FP), false negatives (FN, and true negatives (TN) \cite{singh_diagnosing_2021}. From these, precision [TP/(TP+FP)], recall [TP/(TP+FN)] and their harmonic mean; F1 score were computed. Furthermore, following \cite{troglio_hybrid_2026}, the false positive per true positive [FP/TP] ratio was also reported as an indicator of the false positive burden. 
\subsection{ROC-AUC and PR-AP}
ROC-AUC analysis was used to evaluate the quality of the classifiers, with the area under the ROC curve (AUC) as an indicator of discriminatory power to measure the model's ability to separate classes \cite{hanley_meaning_1982}. PR-AP (Average Precision / Area Under the Precision-Recall Curve) is used to evaluate the quality of positive predictions by focusing on precision and recall 
Given that the target class is a minority of the candidate set, ROC is used because it is invariant to class imbalance with a fixed 0.5 chance level. However, according to \cite{krzhizhanovskaya_model_2020, davis_relationship_2006}, if there is a large skew in the class distribution, ROC can look optimistic about the model's performance. Therefore, PR-AP (average precision) was also used because precision-recall analysis can reveal poor performance under class imbalance that ROC curves alone may hide \cite{krzhizhanovskaya_model_2020}. 
ROC-AUC values were interpreted following \cite{hosmer_applied_2013}: an AUC of 0.5 indicates no discrimination, meaning equivalent to random guessing, and a higher value indicates stronger discrimination. Whereas PR-AP was interpreted against the prevalence baseline, y=P/(P+N), where P is the number of targets and N the number of non-targets \cite{saito_precision-recall_2015}. A PR-AP near the baseline is therefore indicating performance close to chance, while well above the baseline indicates identification of the target class. 
[I feel like this part is becoming a discussion; maybe I need to change it]

\subsection{Bootstrap confidence intervals (single arm)}
Each metric is a single point estimate from one set of candidates. So, due to this finite sample, the uncertainty is quantified with 95\% confidence intervals computed by bootstrap resampling \cite{efron_resampling_2001}, as used for spike sorting performance by \cite{troglio_hybrid_2026}. The candidate set was resampled with replacement 2000 times, and for each resample, the metrics were recomputed using the out-of-fold predictions and scores. Then, the 95\% intervals were taken as the 2.5th and 97.5th percentiles of the resampled values. Any resamples that contained only one class were skipped.  The CIs were computed for all the reported metrics F1, ROC-AUC, PR-AP). The confidence interval reflects the uncertainty around these metrics. For ROC-AUC values, a narrow interval indicates AUC is likely accurate, while a wide interval indicates the AUC value is less reliable \cite{corbacioglu_receiver_2023}; for PR-AP, the corresponding reference is the prevalence baseline, and for F1, the interval conveys the uncertainty around the point estimate. 
\section{After denoising evaluation design}
To separate the effect of denoising on detection from its effect on classification, denoising was applied at two independent stages. The signal used to detect the candidate and the signal from which the classification waveforms were extracted (the cutout). This gives a 2x2 ablation design. The four resulting conditions (arms) are as follows:
\begin{itemize}
    \item \textbf{Full raw}: Raw detection, raw cutout (the baseline with no denoising)
    \item \textbf{Full Denoised}: Denoised detection, denoised cutout (The full denoised pipeline)
    \item \textbf{Hybrid A}: Denoised detection, raw cutout (denoising aids detection only)
    \item \textbf{Hybid B}: Raw detection, denoised cutout (denoising aids classification only.     
\end{itemize}
Each arm isolates where in the pipeline denoising benefits; comparing them shows whether any benefit comes from better detection, better classification features, both or neither. 
For the arms to be comparable, the same detection parameters and latency window were applied across all conditions, so the only difference between arms is the signal used, not changes in the pipeline. Each arm was evaluated by running both classifiers and computing the metrics with confidence intervals. 
The single-arm confidence interval tests whether each arm differs from chance, but not whether two arms differ from each other. To test whether small differences between arms are real differences or within sampling noise, a paired bootstrap was used for this section and computed for ROC-AUC, PR-AP and F1. Only arms that shared the same detection set and differed only in the cutout signal were compared, so that both were evaluated under the same conditions; the two contrasts in this design are: under raw detection (raw versus denoised cutout) and under denoised detection (denoised versus raw cutout) 
For each of 2000 resamples, the same resampled candidates were scored under both arms, and the difference in the metrics was recorded. The 95\% interval was taken of these differences. The candidate-to-candidate variability is cancelled out as both arms are evaluated on the same resampled candidates, which isolates the effect of the cutout signal. This makes the paired test more sensitive to small differences than comparing two separate intervals \cite{du_when_2025}.
A difference in intervals that exclude 0 means there is a real difference between arms; it includes 0, it means the difference is within sampling noise. 
\section{Leakage safe evaluation protocol}
The pipeline described in sections 3.1-3.7 was tuned and evaluated on the same 900-second window: the template, the denoisers and the detection parameters were all derived from the same data used for evaluation. This can produce optimistic performance estimates. To obtain an unbiased estimate, a leakage-safe evaluation was performed on a held-out portion of the recording.  
The recording was divided chronologically into a development set of the first 70\% [3602.7-4232.7 s], which contained 166 target spikes and a held-out evaluation set of the last 30\% [4232.7-4502.7 s], which contained 61 target spikes. All tuning used the development set; the evaluation set was scored once. 
All component that could carry information across the split was fitted on the development set only. So, the spike template was built from development annotations only; the denoisers were trained on synthetic data that is derived from the development template, except wavelet, as the wavelet transform has no trainable component.  The detection parameters like shape similarity threshold and latency window were selected by maximising F1 on the development set. 
For the classifiers, the aligned raw waveform (90 samples, trough-aligned) was cut from the cutout signal and used directly as the feature vector, following \cite{troglio_hybrid_2026}. The classifier and its scaler were fitted on the development survivors only, with fixed hyperparameters and scored once on the evaluation survivors. The confidence intervals were computed by stratified bootstrap resampling of the evaluation candidates.
To note, for the held-out selection, different parameters were chosen (shape threshold [0.2], latency half width [2 ms]) from the exploratory analysis in the sections below (0.3, 4ms).

\chapter{Single-Channel Results}
\section{Experimental setup}
The analysis used a 900-second recording from channel 19, sampled at 30kHz. Within this window, 177 target spikes were identified as ground truth from the fibre of interest, with a median post-stimulus latency of 100.1ms. The raw signal had a noise level (MAD) of 4.958.
 
\section{Detection performance}
\subsection{Held-out evaluation}
The shape similarity and latency window were swept, and the combination chosen was the one that maximised F1 on the development period \ref{tab:dev-sweep}. The shape threshold chosen was 0.2 and a latency window of 4 ms (2 ms half window). 
These parameters were then applied to the held-out evaluation period and scored once against the evaluation. On raw signal, detection showed 68 survivors, of which 18 were true positives and 50 false positives, giving a precision of 0.265 and a recall of 0.295 and an F1 of 0.279. The false positives to true positives ratio was 2.78. 
\begin{table}[htbp]
    \centering
    \begin{tabular}{cccccc}
    \toprule
    Shape threshold & Latency half-width (ms) & Kept & Precision & Recall & F1 \\
    \midrule
    \textbf{0.2} & \textbf{2} & \textbf{114} & \textbf{0.553} & \textbf{0.543} & \textbf{0.548} \\
    0.3 & 2 & 103 & 0.544 & 0.483 & 0.511 \\
    0.4 & 3 & 86 & 0.372 & 0.276 & 0.317 \\
    0.5 & 5 & 28 & 0.321 & 0.078 & 0.125 \\
    \bottomrule
    \end{tabular}
    \caption{Detection parameter selection on the development set. The shape similarity threshold and latency half-width window were swept; the combination selected for the held-out evaluation that maximised F1 were (shape 0.2, half-width 2 ms).}
    \label{tab:dev-sweep}
\end{table}

\subsection{Exploratory/ Development analysis}
The candidate spikes used in the pipeline were detected by template matching, retaining peaks that are above a shape similarity threshold, separated by a refractory period and falling within a latency window. 
The shape similarity threshold was swept from 0.2 to 0.7 \ref{tab: Shape sweep} and the F1 score peaked at a threshold of 0.3, which was selected. Lower thresholds detect more spikes without improving precision, while higher thresholds reject too many detections, leading to a lower recall \ref{tab: Shape sweep}. 
Detection achieved high recall but at the cost of low precision and a false-positive-to-true-positive ratio of ~2\ref{tab: Baseline detection metrics}. To survive detection, a candidate must match the target template waveform shape and fire within the target's latency window.
\begin{table}[htbp]
    \centering
    \begin{tabular}{cccccc}
    \toprule
     Shape threshold & Detections & Precision & Recall & F1 \\
     \midrule
      0.2 & 353 & 0.323 & 0.644 & 0.430 \\ 
      \textbf{0.3} & \textbf{329} & \textbf{0.334} & \textbf{0.621} & \textbf{0.435} \\
    ...
    \bottomrule
    \end{tabular}
    \caption{Shape similarity threshold sweep for spike detection with the latency window applied. The F1 score peaks at a threshold of 0.3, which was selected for the pipeline.}
    \label{tab: Shape sweep}
\end{table}

The latency window was widened symmetrically around the ground truth spikes' median latency of 100.1 ms \ref{tab:latency sweep}. The F1 score peaked at an 8 ms window, which was selected. Narrow windows miss target spikes, leading to a low recall, and a wider window detects more false positives, leading to lower precision. 
\begin{table}[htbp]
    \centering
    \begin{tabular}{cccccc}
    \toprule
    window (ms) & Width (ms) & kept & Precision & Recall & F1 \\
    \midrule
    ...
    \textbf{96.1--104.1} & \textbf{8} & \textbf{329} & \textbf{0.334} & \textbf{0.621} & \textbf{0.435} \\
    95.1--105.1 & 10 & 400 & 0.300 & 0.678 & 0.416 \\
    94.1--106.1 & 12 & 490 & 0.276 & 0.763 & 0.405 \\
    93.1--107.1 & 14 & 566 & 0.253 & 0.808 & 0.385 \\
    \bottomrule
    \end{tabular}
    \caption{Latency window sweep for spike detection. The window is centred on the median latency (100.1 ms) and widened symmetrically. The F1 score peaks at an 8 ms window (half-width of 4 ms, 96.1 - 104.1ms). which was selected for the pipeline}
    \label{tab:latency sweep}
\end{table}
These selected parameters on raw signal detection produced 329 candidate detections \ref{tab:Beseline detection metrics}. The recall was 0.621, so most of the target spikes were detected, but the FP/TP ratio was 1.99, so roughly ~2 false positives are detected for every true positive. Because these false positives passed both the shape similarity threshold and the latency window, indicating that these false positives resemble the target spikes in both waveform shape and firing latency, which cannot be distinguished by template matching with latency constraint alone. This is a similar finding of \cite{troglio_hybrid_2026}. Therefore, the classification stage will address whether these false positives can be separated from the target spikes by waveform shape.  
\begin{table}[htbp]
    \centering
    \begin{tabular}{ccccccc}
    \toprule
    TP & FP & FN & Precision & Recall & F1 & FP/TP \\
    \midrule
    110 & 219 & 67 & 0.334 & 0.621 & 0.435 & 1.99 \\
    \bottomrule
    \end{tabular}
    \caption{Detection performance on the raw signal, using the selected parameters: shape threshold 0.3, 3 ms refractory, 8 ms latency window. Reported following \cite{troglio_hybrid_2026} including the false-positive-to-true-positive ratio (FP/TP).}
    \label{tab: Baseline detection metrics}
\end{table}

\section{Denoising quality}
\subsection{Held-out evaluation}
The denoising quality was assessed by the noise level (MAD) and the peak-to-peak relative to the raw signal. All three denoisers reduced the noise level \ref{tab:denoising-quality-heldout}. When comparing the peak-to-peak amplitude across the target spikes showed that dense retained only 18\% of the raw amplitude, wavelet 32\% and convolutional 66\%. The figure~\ref{fig:denoising} shows this in detail view; the raw and convolutional spikes remain deep while the wavelet and dense spikes are visibly attenuated. 
A similar pattern was observed with the development analysis; dense was the most destructive. This shows that the effect is a general property of the denoising, not a consequence of one particular training setup.
\begin{table}[htbp]
    \centering
    \begin{tabular}{lccc}
    \toprule
    Signal & MAD & Peak-to-peak amplitude & Amplitude retained  \\
    \midrule
    Raw & 4.958 & 24.07 & 100\%  \\
    ...
    \bottomrule
    \end{tabular}
    \caption{Denoising quality (leakage-safe denoisers). Each denoiser reduces the noise level (MAD). With peak-to-peak relative to raw, convolutional autoencoder preserved the most amplitude; dense the least. }
    \label{tab:denoising-quality-heldout}
\end{table}
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/denoising_quality_leakage_safe.png}
    \caption{Effect of each denoiser on the signal (leakage-safe denoisers). Left is an overview of a segment around a target spike. Right is a detail view of the spike.}
    \label{fig:denoising}
\end{figure}
\subsection{Exploratory/ Development analysis}
The noise level of each signal was measured by its MAD. All denoisers reduced the noise significantly relative to the raw signal \ref{tab:denoising quality}, with dense achieving the lowest MAD of 0.559 and wavelet the highest MAD of 2.253. When comparing peak-to-peak amplitude across the 177 target spikes, wavelet and dense denoising reduce the spike amplitude significantly, retaining only about 32\% and \%18 of the raw spike amplitude, respectively. Convolutional denoising, on the other hand, preserved the spike, retaining 54\% while reducing noise significantly, which can also be seen in figure \ref{fig:denoising}. 
\begin{table}[htbp]
    \centering
    \begin{tabular}{lccc}
    \toprule
    Signal & MAD & Peak-to-Peak amplitude & Spike retained \\
    \midrule
    Raw & 4.958 & 23.92 & 100\% \\
    Wavelet & 2.253 & 7.59 & 32\% \\
    Dense & 0.559 & 4.40 & 18\% \\
    Conv & 0.968 & 13.00 & 54\% \\
    \bottomrule
    \end{tabular}
    \caption{Denosing quality. Each denoiser reduces the noise level (MAD). The mean spike-trough shows that wavelet and dense denoising reduce it significantly, while convolutional autoencoder preserves it. The spike retention is the mean trough relative to the raw signal.}
    \label{tab:denoising quality}
\end{table}

\section{The effect of denoising on detection}
\subsection{Held-out evaluation}
This section tests whether denoising the signal improves spike detection. Detection was run on the raw signal and each denoised signal and compared. 
On the held-out evaluation \ref{tab:denoising-detetcion-heldout}, denoising did not improve detection. None of the denoisers increased the recall relative to raw and the false positive to true positives ratio remained above 1 for every signal (1.6-3.5). Dense gave the worst detection F1 of 0.226 and FP/TP of 3.50.

\begin{table}[htbp]
    \centering
    \begin{tabular}{lcccccccc}
    \toprule
    signal & Kept & TP & FP & FN & Precision & Recall & F1 & FP/TP \\
    \midrule
    ...
    \bottomrule
    \end{tabular}
    \caption{Effect of denoising on detetcion (held-out evaluation). Detetcion was applied to the evaluation period on each signal. Denoising did not increase recall or reduce the FP/TP ratio; the FP/TP ratio remained above 1 for every signal.}
    \label{tab:denoising-detetcion-heldout}
\end{table}
\subsection{Exploratory/ Development analysis}
To test the effect of denoising on detection, the detection pipeline was run the same on the raw signal and each denoised signal, using the same parameters throughout. As seen \ref{}, denoising did not increase the number of true positives or the recall; both remained similar across all signals (TP ~110, recall ~0.61). The number of false positives has increased slightly with denoising, from 219 for raw to 234 for conv, which in turn reduced precision and F1 and raised the FP/TP ratio. This pattern was strongest for the denoisers that significantly lowered the noise level (dense and conv). 

These additional detections were assessed using the standard spike sorting quality metrics\cite{hill_quality_2011}. When comparing the survivors across signals, approximately 82\% of denoised detections matched a raw detection within 2 ms, and the remaining 18\% were at new locations.
These additional detections from the denoised signal were checked to determine whether they were real spikes or denoising artefacts. Their mean waveform in the raw signal showed a clear spike trough of -9.56, compared to the target spikes -11.856; they respected the refractory period, so there were no inter-spike intervals below 1 ms. Their peak-to-peak amplitudes were comparable to the target spikes (median 22.36 vs 23.91). This indicates that the additional detections are actual spikes rather than artefacts created by denoising. [To Do: I need to add a table for the leakage one as well]


\section{Classification performance and the effect of denoising}
\subsection{Held-out evaluation}
The classifiers were trained on the development survivors and scored once on the held-out evaluation survivors, using 2$times$2 detection cutout ablation. Confidence intervals were computed by stratified bootstrap of the evaluation candidates. Because the evaluation set is small, these results are underpowered and are interpreted as consistent with chance, rather than as precise estimates.
For one-class SVM, every ROC-AUC confidence interval included 0.5 across all arms and denoisers, except for wavelet hybrid B, which had an upper bound of 0.443 \ref{tab:classification-heldout}. 
For two-class SVM, ten of the twelve arms had confidence intervals that included 0.5, except for the two arms from the dense denoiser; full denoised, ROC-AUC 0.71; Hybrid B, 0.71, where intervals excluded 0.5. 
These two arms should not be interpreted as dense denoising helps. Because Dense had the fewest evaluation targets of only 14 and the lowest recall, giving the least stable estimate; and with 24 arms tested at 95\% confidence,
No leakage was detected; the denoiser, template, scaler and parameters were verified as development only. Therefore, these arms are most consistent with sampling noise. 
\begin{table}[H]
    \centering
    \small
    \begin{tabular}{lllcc}
    \toprule
    Classifier & Denoiser & Arm & ROC-AUC [95\% CI] & PR-AP [95\% CI] (base) \\
    \midrule
    \multirow{12}{*}{One-class}
     & \multirow{4}{*}{Wavelet}
    ...
    \cmidrule{2-5}
     ...
     & & Hybrid B      & 0.473 [0.313, 0.641] & 0.306 [0.232, 0.502] (0.279) \\
    \cmidrule{2-5}
     & \multirow{4}{*}{Conv}
       & Full raw      & 0.403 [0.249, 0.564] & 0.296 [0.214, 0.491] (0.279) \\
     & & Full denoised & 0.505 [0.353, 0.650] & 0.347 [0.286, 0.491] (0.344) \\
     & & Hybrid A      & 0.491 [0.343, 0.641] & 0.342 [0.284, 0.499] (0.344) \\
     & & Hybrid B      & 0.483 [0.329, 0.640] & 0.304 [0.233, 0.497] (0.279) \\
    \midrule
    \multirow{12}{*}{Two-class}
     & \multirow{4}{*}{Wavelet}
       & Full raw      & 0.307 [0.173, 0.447] & 0.207 [0.189, 0.273] (0.279) \\
     & & Full denoised & 0.570 [0.406, 0.731] & 0.509 [0.387, 0.715] (0.385) \\
     & & Hybrid A      & 0.484 [0.323, 0.641] & 0.394 [0.310, 0.540] (0.385) \\
     & & Hybrid B      & 0.472 [0.333, 0.613] & 0.262 [0.225, 0.375] (0.279) \\
    \cmidrule{2-5}
     & \multirow{4}{*}{Dense}
       & Full raw      & 0.307 [0.173, 0.447] & 0.207 [0.189, 0.273] (0.279) \\
     & & Full denoised* & 0.706 [0.538, 0.850] & 0.416 [0.282, 0.679] (0.222) \\
     & & Hybrid A      & 0.481 [0.292, 0.663] & 0.266 [0.185, 0.487] (0.222) \\
     & & Hybrid B*     & 0.728 [0.595, 0.851] & 0.509 [0.373, 0.736] (0.279) \\
    \cmidrule{2-5}
     & \multirow{4}{*}{Conv}
       & Full raw      & 0.307 [0.173, 0.447] & 0.207 [0.189, 0.273] (0.279) \\
     & & Full denoised & 0.412 [0.275, 0.553] & 0.339 [0.262, 0.476] (0.344) \\
     & & Hybrid A      & 0.319 [0.193, 0.456] & 0.262 [0.238, 0.338] (0.344) \\
     & & Hybrid B      & 0.505 [0.367, 0.647] & 0.276 [0.233, 0.390] (0.279) \\
    \bottomrule
    \end{tabular}
    \caption{Held-out classification performance (2$\times$2 ablation, both classifiers, all denoisers). ROC-AUC and PR-AP are shown with 95\% stratified bootstrap confidence intervals; base is the evaluation base rate (the PR-AP chance level). Intervals excluding 0.5 are marked *. Three of 24 arms exclude 0.5 (two dense two-class arms above chance, one wavelet one-class arm below), consistent with sampling noise.}
    \label{tab:classification-heldout}
\end{table}

\subsection{Exploratory/ Development analysis}
The survivors from detection were classified as target or non-target using two SVM classifiers, a one-class and a two-class SVM. Both were evaluated by 5-fold out-of-fold cross-validation with fixed hyperparameters and performance was evaluated by ROC-AUC, PR-AP, and F1, each with a 95\% bootstrap confidence interval.  
For the one-class SVM, the ROC-AUC across all arms and denoisers remained close to 0.5, and every 95\% confidence interval included 0.5 \ref{tab:oneclass-cis}. This indicates that performance is indistinguishable from chance. 
The two-class SVM showed a similar pattern to the one-class SVM; ROC-AUC also remained near 0.5 with every confidence interval including 0.5. Therefore, both classifiers were at chance across all conditions. 
\begin{table}[htbp]
    \centering
    \small
    \begin{tabular}{llccc}
    \toprule
    Denoiser  & Arm & F1 [95\% CI] & ROC-AUC [95\% CI] & PR-AP [95\% CI] \\
    \midrule
    \multirow{4}{*}{Wavelet}
    ...
    & Hybrid A & 0.472 [0.4.4, 0.532] & 0.519 [0.453, 0.585] & 0.356 [0.294, 0.435] \\
    & Hybrid B & 0.487 [0.423, 0.548] & 0.507 [0.444, 0.570] & 0.344 [0.287, 0.418] \\ 
    \bottomrule
    \end{tabular}
    \caption{One-class SVM classification performance across 2$\times$2 ablation for each denoiser. Each metric is shown with its 95\% bootstrap confidence interval. Every ROC-AUC interval includes 0.5, indicating performance can not be distinguished from chance.} 
    \label{tab:oneclass-cis}
\end{table}
\begin{table}[H]
    \centering
    \begin{tabular}{llccc}
    ...
    \midrule
    \multirow{4}{*}{Dense}
    & Full raw & 0.349 [0.266, 0.427] & 0.484 [0.219, 0.548] & 0.353 [0.292, 0.444] \\
    ...
    \bottomrule
    \end{tabular}
    \caption{Two-CLass SVM classification performance across the 2$\times$2 ablation, for each denoiser, with 95\% bootstrap confidence intervals. Every ROC-AUC interval includes 0.5.} 
    \label{tab:Twoclass-cis}
\end{table}

The confusion metrics table below \ref{tab:confusion} shows why classifiers are at chance. In every arm, the false positives to true positives ratio is more than 1, which means that the classifier is accepting more non-target than target candidates. Both classifiers accept targets and non-targets at similar rates, which explains the at-chance ROC-AUC. 
\begin{table}[htbp]
    \centering
    \begin{tabular}{llcccccccc}
    \toprule
    Classifier & Arm & TP & FP & FN & TN & FP/TP & precision & Recall & F1 \\
    \midrule
    \multirow{4}{*}{One-class}
    & Full raw & 74 & 135 & 39 & 81 & 1.82 & 0.354 & 0.655 & 0.460 \\
    & Full denoised & 86 & 163 & 32 & 62 & 1.90 & 0.345 & 0.729 & 0.469 \\
    & Hybrid A & 79 & 138 & 39 & 87 & 1.75 & 0.364 & 0.669 & 0.487 \\
    & Hybrid B & 84 & 148 & 29 & 68 & 1.76 & 0.362 & 0.743 & 0.487 \\
    \midrule
    \multirow{4}{*}{Two-class}
    ...
    \bottomrule
    \end{tabular}
    \caption{Confusion matrix counts and derived metrics for the convolutional denoiser for both classifiers. The false positives to true positive ratio (FP/TP) exceeds 1 in every arm, which indicates more false positives than true positives.}
    \label{tab:confusion}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{lllcc}
    \toprule
    Denoiser & Detection & Metric & $\Delta$ & 95\% CI\\
    \midrule
    Conv & Conv & ROC-AUC & +0.091 & [+0.012, +0.170]* \\
    Conv & Conv & F1 & +0.091 & [+0.003, +0.180]* \\
    Conv & Conv & PR-AP & +0.070 & [+0.011, +0.133]* \\
    \bottomrule
    \end{tabular}
    \caption{Paired bootstrap difference for two-class SVM. A 95\% CI excluding zero (*) indicates a significant difference. Out of all denoisers, the convolutional denoiser under its own detection shows a significant effect, where the raw cutout outperforms the denoised cutout.}
    \label{tab:Paired-twoclass}
\end{table}
