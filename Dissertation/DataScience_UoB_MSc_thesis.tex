...
\subsection{Development analysis}
...
\subsection{Held-out evaluation}
The denoising quality was assessed by the noise level (MAD) and the peak-to-peak relative to the raw signal. All three denoisers reduced the noise level \ref{tab:denoising-quality-heldout}. When comparing the peak-to-peak amplitude across the target spikes showed that dense retained only 18\% of the raw amplitude, wavelet 32\% and convolutional 66\%. The figure~\ref{} shows this in detail view; the raw and convolutional spikes remain deep while the wavelet and dense spikes are visibly attenuated. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/denoising_quality.png}
    \caption{Effect of each denoiser on the signal. Left is an overview of a segment around a target spike. Right is a detail view of the spike.}
    \label{fig:denoising}
\end{figure}
