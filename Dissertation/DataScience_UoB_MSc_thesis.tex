...
This chapter describes the data and the methods used to detect and denoise the target unit's spikes in a single channel. It first introduces the recording and how the target unit is identified, then estimates the noise floor. It then describes the detection pipeline: building the spike template, matching it to the signal, and constraining detections by their timing after the stimulus, followed by the supervised classifier. Finally, it describes the denoising methods evaluated and the metrics used to assess both detection and denoising. 
...
During the recording, an electrical stimulus (Pain stimulus) was delivered repeatedly, giving 2,310 stimulation pulses (TTL) at a nominal rate of around 0.25 Hz. The target fibre responds to each stimulus by firing at an approximately fixed latency. The ground truth spike times were marked manually by a human annotator, who only labelled recognisable nerve responses;  therefore, the annotations are conservative, and not every response is labelled. Each spike is marked at the point where the signal crosses zero on its descending slope. 
This project develops the pipeline primarily on Channel 19, a channel with a clear recognisable ground truth and uses other channels to assess consistency across channels. 
...
\end{equation}
Where \[x\] is the signal. The median is used instead of the standard deviation because the spikes are large and infrequent, 

and computed as the median of the absolute signal divided by 0.6745. The median is used instead of standard deviation because the spikes are large and infrequent, so the median does not inflate the estimate the way standard deviation would; dividing by 0.6745 rescales the value to be comparable to a standard deviation under an assumption of Gaussian noise \cite{drebitz_optimizing_2019}. MAD is an estimate, so it assumes the signal is mostly noise and approximately Gaussian \cite{coupe_robust_2010}. But because it is applied identically to every signal, it is a valid basis for comparison. This noise estimate forms the basis of the amplitude detection threshold described in the sections below. 
\section{Template building and spike detection}
The spike template is built by averaging the ground truth spikes of the target unit. For each marked spike, a 60-sample window (2ms at 30kHz) is extracted from the signal, and these windows are averaged to give the unit's mean waveform. Before averaging, each spike is aligned on a common reference point within a short search region around the marked time, so that the same feature falls at the same position in every window; without this, the waveform would sit at a slightly different offset, and the average would be blurred
Two alignment methods are used. The first aligns each spike at its trough (the most negative point), which is a standard and simple approach but is sensitive to noise, since the exact location of the trough can shift slightly from spike to spike \cite{lee_non-linear_2021}. The second method, as described in \cite{caro-martin_spike_2018}, aligns each spike at the negative peak of the signal's first derivative. 
...
Spikes are also detected from their amplitude. Because the target unit's spikes are negative going, a detection is registered wherever the signal crosses below a set threshold relative to the noise floor: 
\begin{equation}
    \text{threshold} = -k,\sigma_{\text{noise}}
\end{equation}
Where \(\sigma_{\text{noise}}\) is the median absolute deviation estimate from the previous section and \(k\) is a multiplier controlling how strict the threshold is \cite{quiroga_unsupervised_2004, park_deep_2019}. A downward crossing below this threshold marks a candidate spike. To prevent a single spike being counted more than once, a 3 ms refractory period is enforced after each detection, during which no further detection is registered. This is done to prevent a single spike from being counted more than once, because a single action potential lasts several milliseconds and can cross the threshold more than once as its waveform falls and rebounds, producing multiple detections of the same spike. The reason the refractory period is longer than what is usually reported (0.66 ms in \cite{laboy-juarez_normalized_2019}; 1.28ms in \cite{rizk_optimizing_2009}) is that when intervals between raw detections were measured, the duplicate detections were separated by a median of only 1 ms, which is far shorter than the interval between real spikes from the target unit, which fires roughly once per stimulus. A 3 ms refractory period therefore removes these repeats while leaving real spikes unaffected. 
\section{The latency constraint}
\section{Spike classification (SVM)}
...
\section{Why detection is limited: the overlap diagnostic}
\section{Denoising quality and the metric trap}
\section{The effect of denoising on detection}
\section{Consistency across channels}

\chapter{Multi-Channel Methods}
\section{The case for multiple channels}
\section{Propagation-delay alignment}
\section{Cross-channel fingerprint}

\chapter{Multi-Channel Results}
